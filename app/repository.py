...
async def get_profile(db: AsyncClient, user_id: str) -> dict[str, Any] | None:
    response = await (
        db.table("profiles")
...
async def get_profiles(db: AsyncClient, user_ids: Iterable[str]) -> dict[str, dict[str, Any]]:
    """Fetch several profiles at once, keyed by id — one round trip, not N."""
    ids = list({str(uid) for uid in user_ids})
    if not ids:
        return {}
    response = await db.table("profiles").select(PROFILE_COLUMNS).in_("id", ids).execute()
